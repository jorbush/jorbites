...
├── components/
│   ├── drafts/
│   │   ├── DraftCard.tsx         # Responsive draft card with actions (Manage Collaborators, Duplicate, Delete) and live metadata
...
│       ├── DraftInviteModal.tsx  # In-app invite management (Invite Link and direct User Search below link), link copy/regeneration, role toggle, collaborator remove/add
...
│           └── RelatedContentStep.tsx    # Content associations (Linked Recipes, Quests, Videos); co-cooks isolated to DraftInviteModal
```
...
Defined in [`app/utils/constants.ts`](../app/utils/constants.ts):
...
13. **In-App Invite Management & Direct User Search (D-08)**:
    - Dedicated `DraftInviteModal` opened via `DraftCard`'s manage collabs icon (keeping `RecipeModalTopActions` clean and completely isolating co-cook management from `RelatedContentStep`). Features a unified single view displaying the **Invite Link** directly (generated automatically on modal mount without requiring an explicit "Generate Invite Link" button) alongside link copy and regeneration (`POST /api/draft/invite`). Situated directly below the link is **Direct User Search**, allowing owners to search and add co-cooks via `POST /api/draft/collaborator`.
    - **Solo-to-Shared Auto-Promotion**: If an owner searches and adds a collaborator to a draft that is currently a solo draft, `DraftService.addCollaborator` (and `POST /api/draft/invite`) automatically promotes the solo draft from `draft:user:${userId}:${draftId}` into a shared collaborative draft (`draft:shared:${draftId}`) with a minted `inviteToken` and cleans up the solo draft key, preventing 404 errors.
...
16. **Step Conflict Detection & Safe Auto-Apply (D-09)**:
    - `detectStepConflict` detects when a remote co-cook modifies fields on the active step.
    - Untouched fields on the active step are auto-applied directly into the form without requiring manual refresh button clicks.
    - Active uncommitted local modifications (keystrokes in progress) are strictly protected against remote overwrite, preventing race conditions.
    - Notifies the collaborator using a standard application toast (`Step {{stepNumber}} updated by a co-cook` with icon `👨‍🍳` and deduplication ID), eliminating broken floating refresh overlays.
...
| [`draftSyncUtils.conflict.test.ts`](file:///__tests__/unit_test/utils/draftSyncUtils.conflict.test.ts) | Vitest | Active step conflict detection (`detectStepConflict`), uncommitted local edits vs remote incoming changes, and conflict resolution via `forceApplyStepFields`. |
...
| [`DraftInviteModal.test.tsx`](file:///__tests__/unit_test/components/modals/DraftInviteModal.test.tsx) | Vitest | In-app invite management, direct invite link display without manual generate button, automatic invite token request on mount, copy & regenerate link, role toggle (Editor vs Viewer), remove collaborator, and user search section directly below link. |
| [`useDraftInviteModal.test.ts`](file:///__tests__/unit_test/hooks/useDraftInviteModal.test.ts) | Vitest | Zustand modal store state transitions for opening, setting active draftId, and closing `DraftInviteModal`. |
...
| [`useRecipeRelatedContent.test.ts`](file:///__tests__/unit_test/components/modals/recipe-steps/useRecipeRelatedContent.test.ts) | Vitest | Linked recipe addition/removal, quest selection, and draftData derivation (with co-cooking isolated to DraftInviteModal). |
...
| [`draftCollaborator.test.ts`](file:///__tests__/unit_test/routes/draftCollaborator.test.ts) | Jest | Add/remove collaborator routes, solo-to-shared automatic promotion when adding collaborator directly via search, and quota limits. |
| [`draftRole.test.ts`](file:///__tests__/unit_test/routes/draftRole.test.ts) | Jest | Role management endpoint (`PATCH /api/draft/role`), owner-only guard, and lock release upon switching to viewer. |

### 2. End-to-End Tests (Cypress)

| Spec File | Test Cases & Key User Flows |
|---|---|
| [`collaborative_roles_invites.cy.ts`](file:///__tests__/e2e/collaborative_roles_invites.cy.ts) | 1. **In-App Invite Link**: Opens `DraftInviteModal` from `DraftCard` and copies auto-generated invite link.<br/>2. **Invite Regeneration**: Allows owner to regenerate invite link with confirmation and rotates token.<br/>3. **Role Management**: Owner toggles co-cook role between `editor` and `viewer`, verifies read-only banner and inert controls for viewer.<br/>4. **Collaborator Removal**: Allows owner to kick a co-cook collaborator from the draft roster.<br/>5. **Direct User Search Addition**: Allows owner to add collaborator directly via user search in `DraftInviteModal`.<br/>6. **Search Addition Edit Access**: Verifies that a collaborator added via search can open the draft in `RecipeModal` and successfully edit recipe content.<br/>7. **Solo-to-Shared Auto-Promotion**: Verifies adding a collaborator directly via search on a pure solo draft auto-promotes it to shared without 404.<br/>8. **Viewer Mode Restrictions**: Verifies a co-cook with viewer role sees the view-only banner, has inputs marked `inert` and `pointer-events-none`, and save draft button is disabled.<br/>9. **Collaborator Read-Only View & Self-Leave**: Verifies that a non-owner collaborator cannot regenerate links, add users, or change roles, and can click "Leave draft" to cleanly exit the collaborative draft.<br/>10. **Token Invalidation on Regeneration**: Verifies that rotating an invite token invalidates the old token (rejecting join attempts) while allowing seamless joining via the new token.<br/>11. **Max Co-Cooks Quota Lockout**: Enforces maximum of 4 co-cooks in `DraftInviteModal` with disabled search and warning message, which dynamically re-enables when a collaborator is removed.<br/>12. **Kicked Collaborator Access Revocation**: Verifies that removing a collaborator immediately revokes their access, purges the draft from their "My Drafts" modal, and blocks direct API access.<br/>13. **RelatedContentStep Cleanliness**: Confirms `RelatedContentStep` contains only Linked Recipes, Quests, and Videos without stale co-cooks tab or share buttons. |
