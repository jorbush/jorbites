...
| `SHARED_DRAFT_POLL_INTERVAL_MS` | `8000` (8s) | Polling interval for navbar active shared draft banner |
...
|----------|--------|-------------|
| `/api/draft` | `GET`, `POST`, `DELETE` | Manages single-user or shared drafts via `DraftService`. Sanitizes tokens for non-owners. |
