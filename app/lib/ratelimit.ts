    ...
    return {
        limit: async (identifier: string) => {
            if (process.env.NODE_ENV !== 'production') {
                logger.info(`[Ratelimit Disabled] Mock limit on ${name} for ${identifier}`);
            }
            return {
                success: true,
                limit: 100,
...
export const authenticatedRatelimit = hasUpstashConfig ? new Ratelimit({
    redis: Redis.fromEnv(),
...
export const unauthenticatedRatelimit = hasUpstashConfig ? new Ratelimit({
    redis: Redis.fromEnv(),
...
export const registrationRatelimit = hasUpstashConfig ? new Ratelimit({
    redis: Redis.fromEnv(),
...
export const passwordResetRatelimit = hasUpstashConfig ? new Ratelimit({
    redis: Redis.fromEnv(),
...
export const contentCreationRatelimit = hasUpstashConfig ? new Ratelimit({
    redis: Redis.fromEnv(),
