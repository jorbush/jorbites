...
/**
 * Fetches the current active challenge. Returns null if none exists.
 */
export async function fetchCurrentChallenge(): Promise<WeeklyChallenge | null> {
    logger.info('fetchCurrentChallenge - start');
    const now = new Date();
    ...
    if (currentChallenge) {
        logger.info('fetchCurrentChallenge - found existing challenge', {
            challengeId: currentChallenge.id,
        });
        return currentChallenge as WeeklyChallenge;
    }

    logger.info('fetchCurrentChallenge - no active challenge found');
    return null;
}
 ...
 */
export async function generateNewChallenge(): Promise<WeeklyChallenge> {
    ...
    const templateList = (challenges as any)[
    ...
    switch (randomType) {
        ...
        case 'dietary':
            ...
            break;
        default:
            throw new Error(`Invalid challenge type: ${randomType}`);
    }
