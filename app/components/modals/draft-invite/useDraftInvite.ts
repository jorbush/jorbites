...
async function generateInitialDraftToken(draftId: string) {
    try {
...
export function useDraftInvite(currentUser?: SafeUser | null) {
    ...
    const autoGeneratedDraftIdRef = useRef<string | null>(null);

    useEffect(() => {
        if (!isOpen) {
            autoGeneratedDraftIdRef.current = null;
            return;
        }
        ...
        if (
            ...
            draft &&
            !effectiveToken &&
            autoGeneratedDraftIdRef.current !== draftId
        ) {
            autoGeneratedDraftIdRef.current = draftId;
            generateInitialDraftToken(draftId).then(async (data) => {
                if (data?.draft) {
                    await mutateDraft(data.draft, false);
                } else if (data) {
                    await mutateDraft();
                } else {
                    autoGeneratedDraftIdRef.current = null;
    ...
    }, [isOpen, draftId, isOwner, draft, effectiveToken, mutateDraft]);
