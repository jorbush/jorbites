...
describe('Recipes API Error Handling', () => {
    ...
    it('should accept valid YouTube URLs', async () => {
        ...
        for (const youtubeUrl of validYouTubeUrls) {
            const request = new NextRequest(
                'http://localhost:3000/api/recipes',
                {
                    ...
                    body: JSON.stringify({
                        title: 'Test title',
                        description: 'Test description',
                        imageSrc: `test-${Date.now()}-${Math.random()}.jpg`, // Unique image to avoid conflict
                        category: 'test',
                        method: 'test',
                        ingredients: ['test'],
                        steps: ['test'],
            ...
            const data = await response.json();
            expect(data.youtubeUrl).toBe(youtubeUrl);
        }
    });

    it('should accept empty YouTube URL', async () => {
        mockedSession = {
            expires: 'expires',
            user: {
                name: 'test',
        ...
        const request = new NextRequest('http://localhost:3000/api/recipes', {
            ...
            body: JSON.stringify({
                title: 'Test title',
                description: 'Test description',
                imageSrc: `test-empty-youtube-${Date.now()}.jpg`,
                category: 'test',
                method: 'test',
                ingredients: ['test'],
                steps: ['test'],
        ...
        const data = await response.json();
        expect(data.youtubeUrl).toBe(null);
    });
    ...
    it('should trim whitespace from YouTube URL', async () => {
        ...
        const request = new NextRequest('http://localhost:3000/api/recipes', {
            ...
            body: JSON.stringify({
                title: 'Test title',
                description: 'Test description',
                imageSrc: `test-trim-youtube-${Date.now()}.jpg`,
                category: 'test',
                method: 'test',
                ingredients: ['test'],
                steps: ['test'],
