...
import QuestsClient from '@/app/courses/quests/QuestsClient';
...
describe('QuestsClient', () => {
    ...
    it('renders the course layout and initial overview step', () => {
        render(<QuestsClient currentUser={null} />);

        // Layout elements
        expect(screen.getByText('course_quests')).toBeDefined();
        expect(
            screen.getByText('quests_course_details.course_description')
        ).toBeDefined();
        ...
        expect(screen.getAllByText('linking').length).toBeGreaterThan(0);
