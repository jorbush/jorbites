import { render, screen, fireEvent, cleanup } from '@testing-library/react';
import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import RecipeCreatorOverview from '@/app/courses/recipe-creator/RecipeCreatorOverview';
import React from 'react';

// Mock react-i18next
vi.mock('react-i18next', () => ({
    useTranslation: () => ({
        t: (key: string) => key,
    }),
}));

describe('RecipeCreatorOverview', () => {
    let markModuleCompletedMock: any;
    let onNextMock: any;

    beforeEach(() => {
        markModuleCompletedMock = vi.fn();
        onNextMock = vi.fn();
    });

    afterEach(() => {
        cleanup();
    });

    it('renders labels and description texts correctly', () => {
        render(
            <RecipeCreatorOverview
                completedModules={{}}
                markModuleCompleted={markModuleCompletedMock}
                onNext={onNextMock}
            />
        );

        expect(
            screen.getByText('recipe_creator_course_details.requirements_title')
        ).toBeDefined();
        expect(
            screen.getByText('recipe_creator_course_details.req_inputs_label')
        ).toBeDefined();
        expect(
            screen.getByText(
                'recipe_creator_course_details.req_plaintext_label'
            )
        ).toBeDefined();
        expect(
            screen.getByText('recipe_creator_course_details.req_draft_label')
        ).toBeDefined();
        expect(
            screen.getByText('recipe_creator_course_details.req_editing_label')
        ).toBeDefined();
    });

    it('enables the next step button only when the module is completed', () => {
        const { rerender } = render(
            <RecipeCreatorOverview
                completedModules={{}}
                markModuleCompleted={markModuleCompletedMock}
                onNext={onNextMock}
            />
        );

        // Next button is disabled initially
        const nextBtn = screen.getByRole('button', {
            name: 'contest_manager_course_details.next_step',
        });
        expect(nextBtn).toBeDefined();
        expect((nextBtn as HTMLButtonElement).disabled).toBe(true);

        // Rerender with requirements completed
        rerender(
            <RecipeCreatorOverview
                completedModules={{ requirements: true }}
                markModuleCompleted={markModuleCompletedMock}
                onNext={onNextMock}
            />
        );

        // Next button should now be enabled
        expect((nextBtn as HTMLButtonElement).disabled).toBe(false);

        // Clicking it triggers onNext callback
        fireEvent.click(nextBtn);
        expect(onNextMock).toHaveBeenCalled();
    });

    it('marks the module completed when all checkboxes are checked', () => {
        render(
            <RecipeCreatorOverview
                completedModules={{}}
                markModuleCompleted={markModuleCompletedMock}
                onNext={onNextMock}
            />
        );

        const checkInputs = screen.getByLabelText(
            'recipe_creator_course_details.checklist_inputs'
        );
        const checkPlainText = screen.getByLabelText(
            'recipe_creator_course_details.checklist_plaintext'
        );
        const checkDraft = screen.getByLabelText(
            'recipe_creator_course_details.checklist_draft'
        );
        const checkEditing = screen.getByLabelText(
            'recipe_creator_course_details.checklist_editing'
        );

        // Check first 3 boxes -> should not complete
        fireEvent.click(checkInputs);
        fireEvent.click(checkPlainText);
        fireEvent.click(checkDraft);
        expect(markModuleCompletedMock).not.toHaveBeenCalled();

        // Check the final box -> should complete requirements module
        fireEvent.click(checkEditing);
        expect(markModuleCompletedMock).toHaveBeenCalledWith('requirements');
    });
});
